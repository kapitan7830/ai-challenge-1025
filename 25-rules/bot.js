...
// Считываем правила для агента
let agentRules = '';
try {
  agentRules = readFileSync(resolve('./rules.md'), 'utf-8');
  logger.success('Правила агента загружены из rules.md');
} catch (error) {
  logger.warn('Не удалось загрузить rules.md, агент будет работать без правил');
}
