...
const ragAgent = new RagAgent(OPENAI_API_KEY, PERPLEXITY_API_KEY);
...
bot.on('text', async (ctx) => {
  ...
  try {
    ...
    const result = await ragAgent.searchWithPerplexityFallback(userQuery);
    ...
    if (result.found) {
      ...
      if (result.source.type === 'perplexity') {
        responseText += `🌐 Источник: Perplexity\n`;
        responseText += `📄 ${result.source.filename}\n`;
      ...
      await ctx.reply('😔 Информация не найдена ни в базе данных, ни в Perplexity.\n\nПопробуйте уточнить запрос.');
    }
  } catch (error) {
